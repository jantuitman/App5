...
/**
  ...
  the components that are parsed are supposed to belong to the view called viewName.
*/
App5Parser.prototype.parse=function(viewName,string,success,failure,root)
{
	var self=this;
...
App5Parser.prototype.loadComponents=function(componentList,z,failure) {
	...
	var url;
	//alert("uri;s  : "+componentObj.uri+" "+App5.URI)
	...
	if (status.success){
		if (element.nodeType==1) {
			if (this.componentClasses[this.pureName(element.nodeName)])  {
				...
				var shortid=id;
				id=App5.genId(this.viewName,id);
				...
				parent.addChild(obj);
				// the parsed object is never a clone and has subid 0.
				App5.ids[id+'_'+obj.subid]=obj;
				// attributes
			...
			if (parent.children.length>0 && parent.children[parent.children.length-1].name=="a5_fragment") {
				...
				var id=App5.genId(this.viewName);
				var fragment=new f(id);
				f.viewName=this.viewName;
