...
{
	this.id=null;
	this.subid='0'; // allways start with 0 as subid.
...
App5Component.prototype.canHaveChild=function(string) {
	// all imported extensions can be inserted everywhere.
	if (string.indexOf(":")>=0) return { success: true }
...
/* extremely simple and stupid callback function for models. 
    to do, override this in list and form components to make it more efficient. (mark only children that need updates).
*/
App5Component.prototype.valueChanged=function(keyPath) {
	this.sendEventToController("modelupdate",this.model);
    App5.markUpdate(this);
}
...
/**
todo: add databinder property to a5_list?
*/
App5Component.prototype.isDataBound=function() {
	if (this.getParentObject() !=null && this.getParentObject().name=="a5_list") {
		return true;
	}
	return false;
}
...
App5Component.prototype.sendEventToController=function(eventName,data) {
	...
	if (o && o.controller) {
		console.log("trying to call on"+eventName+'_'+this.shortid);
		
		if (o.controller['on'+eventName+'_'+this.shortid]) {
			o.controller['on'+eventName+'_'+this.shortid](data);
		}
	}
}
...
App5Component.prototype.detach=function() {
	if (this.model && this.model.removeListener) this.model.removeListener(this);
}

App5Component.prototype.clone=function(subId) {
	var shortId=this.shortid;
	var o=new App5.components[this.name](App5.genId(this.viewName,shortId));
	o.subid=subId;
	o.shortid=shortId;
	App5.ids[o.id+'_'+o.subid]=o;
	...
	for (var j=0;j<this.children.length;j++) {
		if (this.children[j]==null || this.children[j].clone==null) {
			console.error("error, no clone function",this,this.children[j]);
		}
