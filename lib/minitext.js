			...
			else
			{
				...
				else if (startChar=='<')
				{
					if (line.match(/^<<</))
					{
						...
						this.appendBlock("<pre>"+this.replaceHtml(line.substr(3)));
					}
				}
				else if (startChar=='"')
				{
...
MiniText.prototype.replaceCodes=function(s)
{
	...
	// Notebook: adapted minitext here to make local links possible within notebook.
	s=s.replace(/"(.*?)":\(#([^\)]*)\)/g,'<a href="javascript:ctl_link(\'$2\')">$1</a>');
