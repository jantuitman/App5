...
.module('stackview',function(globals) {
	...
	function _resetTranslate(that) {
		...
		that.scroller.css("webkitTransform","translate3d("+(0-currentPos)+"px,0,0)")
	}
	...
	*/
	...
	StackView.prototype.pushView=function(view) {
		...
		view.css({"position" : "absolute", "left" : that.xPos +"px", width: el.width(), height: el.height() });
		that.views.push(view);
		that.scroller.append(view);
		xPos += el.width();
		return _scrollTo(that,that.xPos);
	...
	*/
	StackView.prototype.replaceView=function(view,direction) {
		if (direction=="forward" || direction =="back") {
			...
			view.css({"position" : "absolute", "left" : this.xPos +"px", width: x, height: this.el.height() });
			...
			return _scrollTo(this,this.xPosLeft).done(function () {
				// remove all nodes
				for (var c in this.scroller.children()){
					if (c != view.get(0)) {
						$(c).remove();
					}
				}
		...
		else throw new Error("Illegal argument: direction in jumpToView");
	}
