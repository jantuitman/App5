	...
	*/
	function _replaceCache(that) {
		...
		for (var v in that.position) {
			if (v<that.currentPage-1 || v> that.currentPage+that.options.minCache) {
				that.pages[v].remove();
				delete that.pages[v];
		...
		promise.join(promises).done(function () {
			for (var i = -1;i<that.options.minCache;i++) {
	...
	Pager.prototype.nextPage=function () {
		...
		if (that.cacheCount < that.options.maxCache) {
			_cachePage(that,that.lastPage+1).done(function (page){
		...
		if (that.position[that.currentPage+1] != null) {
			...
			_cachePage(that,that.currentPage+1).done(function (page) {
