	...
	*/
	Promise.prototype.join=function () {
		...
		function resultReceiver(i){
			return function () {
				...
				console.log("received result for "+i+" waitcount = "+waitCount);
				if (waitCount==0) {
					self.fulfill(true);
				}
			}
		...
		if (arguments[0].length) {
			...
			for (var i=0;i<arr.length;i++) {
				console.log(" adding an promise ",i,arr[i]);
				waitFor[i]=1;
			}
			for (var i=0;i<arr.length;i++) {
	...
	Promise.prototype.done=function(obj) {
		if (obj.constructor == Promise) {
			...
			else if (this.failure) obj.apply(null,this.failure);
			else this.callbacks.push({ type: 'any', f: obj })
		}
	}
	...
	Promise.prototype.whenFailed=function(obj) {
		if (obj.constructor == Promise) {
			...
			else if (!this.finised) this.callbacks.push({ type: 'fail', f: obj })
	...
	/**
	   ...
	   so you can do something like this:
	     ...
	     promise.about(function(success,failure) {
				ajaxCall(bla,bla,success,failure)
	    }).and(		function(success,failure) {
						ajaxCall(bla,bla,success,failure)
			    })
	*/
	...
	Promise.prototype.and=function(f) {
		var p=(new Promise()).about(f);
		return this.join(p);
	}
